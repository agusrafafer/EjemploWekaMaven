 ...
 */
public class ArbolRegresion extends AprendizClasificacion{
    
    private ArbolRegresion(){
        
    }
            
    public ArbolRegresion(InputStream is){
        super.setIs(is);
    }
...
//        setInstancias(Filter.useFilter(getInstancias(), rm));
    ...
    public void crearClasificador() throws Exception {
        ...
        if(!attr.isNumeric()){
            System.err.println("El último atributo debe ser numérico para poder construir el modelo de árbol de regresión lineal");
            return;
        } 
        setClasificador(new M5P());
